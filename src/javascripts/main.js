...
let stageBreaks = [
    10, 25, 40, 55, 56, 70, 85, 100, 115, 116, 130, 145, 160, 175, 176, 190, 205, 220, 235, 236, 250, 265, 280, 295,
...
// Initialize latestStage from local storage
let latestStage = parseInt(localStorage.getItem("latestStage")) || 0;
let menuButtons = [
    ...
    // 	}
    // }),
    new Button(RINGWIDTH / 2 + LEFTWALL, 330, 150, 25, "Levels", () => (scene = LEVELS)),
    new Button(RINGWIDTH / 2 + LEFTWALL, 360, 150, 25, "Survival", () => {
        resetGameState({
            playerX: RINGWIDTH / 2 + LEFTWALL,
...
[0].concat(stageBreaks).forEach((stageScore, index) => {
    ...
    levelButtons.push(
        new Button(LEFTWALL + column * 90 + 100, CEILING + 80 + row * 70 + 50, 80, 40, `Stage ${index + 1}`, () => {
            resetGameState({
...
let survivalTutorialScreen = new TutorialScreen(
	...
	Good luck!! 
	Only the best can achieve Glory in the Ring!`,
    SURVIVE
);
    ...
    for (let i = 0; i < enemies.length; i++) {
        ...
        if (!player.hasSaw) {
            if (enemies[i].check(saw.x, saw.y)) {
                enemies.push(
                    new Enemy(random(LEFTWALL + 20, RIGHTWALL - 20), random(CEILING + 20, FLOOR - 20), i, CHASER)
                );
...
function surviveDraw() {
    ...
    if (score >= stageBreaks[stage] && breatheTimer < 0) {
        if (stage != COLUMNSTAGE /*&& stage != BULLETSTAGE*/ && stage != DREVILSTAGE && stage != TWINSTAGE) {
            breatheTimer = 180;
            latestStage = stage + 1;
            localStorage.setItem("latestStage", latestStage.toString());
            resetLevelButtons();
        }
    }
    ...
    if (ttTimer <= 0) {
        ...
        if (
            ...
            (potentialPointCount + score < stageBreaks[stage] ||
                stage == COLUMNSTAGE /*|| stage == BULLETSTAGE*/ ||
                stage == DREVILSTAGE ||
                stage == TWINSTAGE)
        ) {
            if (bosses.length <= 0) {
                    ...
                    if (heldPowerups[i].type == RICOCHET && saw.hitOne == false && enemies.length >= 1) {
                        ...
                        for (let i = 0; i < enemies.length; i++) {
                            if (dist(saw.x, saw.y, enemies[i].x, enemies[i].y) < closestDist && enemies[i].killable) {
                                if (
                                    enemies[i].x >= LEFTWALL &&
                                    enemies[i].x <= RIGHTWALL &&
                                    enemies[i].y >= CEILING &&
                                    enemies[i].y <= FLOOR
                                ) {
            ...
            for (let i = enemies.length - 1; i >= 0; i--) {
                if (
                    dist(
                        enemies[i].x,
                        enemies[i].y,
                        enemies[i].thisPattern[enemies[i].thisPattern.length - 1].x,
                        enemies[i].thisPattern[enemies[i].thisPattern.length - 1].y
                    ) <= 20 &&
                    enemies[i].type == TRACER
                ) {
                    kill(enemies[i].id);
                }
            }
...
export function kill(j) {
    if (
        (enemies[j].type == CHASER ||
            enemies[j].type == STANDER ||
            enemies[j].type == TRACER ||
            enemies[j].type == FROG) &&
        enemies[j].killable &&
        stage != TWINSTAGE
    ) {
